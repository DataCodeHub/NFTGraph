...
class PretrainModule(torch.nn.Module):
    ...
    def p_link(self, sg, pos_u, pos_v, neg_u, neg_v):
        ...
        logits = torch.cat([torch.sigmoid(pos_logits).view(-1), torch.sigmoid(neg_logits).view(-1)])
        target = torch.cat([torch.ones_like(pos_logits).view(-1),torch.zeros_like(neg_logits).view(-1)])
