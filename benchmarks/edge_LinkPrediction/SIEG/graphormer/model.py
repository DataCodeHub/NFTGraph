...
class Graphormer(nn.Module):
    ...
    def forward(self, data, perturb=None):
        ...
        node_feature = torch.zeros((n_graph, n_node, graph_token_feature.size(2))).to(device)
        graph_node_feature = torch.cat(
            [graph_token_feature, node_feature], dim=1).to(device)

        # graph_node_feature = graph_token_feature
        # print("graph_node_feature的size:",graph_node_feature.size())
        # transfomrer encoder
        output = self.input_dropout(graph_node_feature)
        # print("输入进来的size:",output.size())
        # print("graph_attn_bias的size:",graph_attn_bias.size())
