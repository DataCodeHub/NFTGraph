...
from src.data_train import Graph_Dataset, Universal_Collator,NFTSubGraph_Dataset,NFT_Collator
...
def main(rank, world_size, opt):
    ...
    def train(model, optimizer, scheduler, step, opt, checkpoint_path):
        ...
        # nft_collator = NFT_Collator(g, opt.use_saint, opt.per_gpu_batch_size, opt.device, tasks,opt.lp_neg_ratio)
        # dataset = NFTSubGraph_Dataset(g)
        curr_losses = {}
        model.train()
        model.zero_grad()
        inner_step = 0
...
if __name__ == '__main__':
    ...
    opt = options.parse()
    opt.pretrain_label_dir = '/ParetoGNN/pretrain_labels'
    opt.tvt_addr = '/ParetoGNN/links/tinynft_nodetype_tvtEdges.pkl'
    opt.dataset = 'tinynft_nodetype'
    opt.tasks = ['p_link']
    opt.hid_dim = [256,128]
    opt.predictor_dim = 128
    opt.total_steps = 100000
    opt.save_freq = 100000
    # opt.mask_edge = True
    print(opt)
    ...
    if opt.is_distributed:
        ...
        print(f"stime:{stime},etime:{etime},spend:{etime-stime}")