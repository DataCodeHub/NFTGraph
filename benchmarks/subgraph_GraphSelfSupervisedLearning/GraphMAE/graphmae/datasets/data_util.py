...
GRAPH_DICT = {
    ...
    "ogbn-tinynft": DglNodePropPredDataset
}
...
def load_dataset(dataset_name):
    ...
    if dataset_name == 'ogbn-tinynft':
        ...
        cat_dict = torch.load('/data/sx/ogbn-allnft/data/tinynft/submission_ogbn_tinynft_nodetype/meta_dict.pt')
        ...
        graph, labels = dataset[0]
        num_nodes = graph.num_nodes()
        tvt_addr = '/data/sx/SSL/ParetoGNN/links/tinynft_nodetype_tvtEdges.pkl'
        mask_edge = True
        if mask_edge:
            _, _, val_edges, _, test_edges, _ = pickle.load(open(tvt_addr, 'rb'))
            lst = []
            lst.append(graph.edge_ids(val_edges[:,0], val_edges[:,1]))
            # lst.append(g.edge_ids(val_edges[:,1], val_edges[:,0]))
            lst.append(graph.edge_ids(test_edges[:,0], test_edges[:,1]))
            # lst.append(g.edge_ids(test_edges[:,1], test_edges[:,0]))
            lst = torch.cat(lst)
            graph.remove_edges(lst)
