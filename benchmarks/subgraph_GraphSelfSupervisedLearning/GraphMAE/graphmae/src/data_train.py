...
class NFTSubGraph_Dataset(torch.utils.data.Dataset):
    def __init__(self, g):
        self.len = g.edata['feat'][:,0].unique().size()[0]
        self.start = g.edata['feat'][:,0].min().item()
        self.end = g.edata['feat'][:,0].max().item()
        self.eid_tensor = g.edges(form='eid')
        self.edata_tokenid = g.edata['feat'][:,0]
        self.g = g
    ...
    def __getitem__(self, idx):
        ...
        idx += self.start
        assert idx <= self.end, 'OverFlow: NFT idx!'
        eachnft_links = torch.masked_select(self.eid_tensor,(self.edata_tokenid == idx ))
        print(type(idx),idx,'\n',eachnft_links.size())
        return eachnft_links

class NFT_Collator(object):
    def __init__(self, g, use_saint, batch_size, device, tasks,link_negative_ratio):
        self.g = g
        self.use_saint = use_saint
        self.batch_size = batch_size
        self.device = device
        self.tasks = tasks
        self.link_negative_ratio = link_negative_ratio
        if 'p_link' in tasks:
            # link
    ...
    def __call__(self, indices):
        ...
        if 'p_link' in self.tasks:
            # link
            nft_subgraph_indices = indices[0]
            if nft_subgraph_indices.shape[0] <= 1:
                print("nft_subgraph_indices",nft_subgraph_indices)
                try:
                    nft_subgraph_indices = torch.randperm(nft_subgraph_indices[0].item())[:10]
                except:
                    nft_subgraph_indices = torch.randperm(self.g.num_edges()-1)[:10]
            # output_from_mysample = self.mysample(nft_subgraph_indices)
            # outputs['p_link'] = link_prediction_data_process(output_from_mysample, self.device)
            output_from_dataloader = self.link_sampler.sample(self.g, nft_subgraph_indices)
            outputs['p_link'] = link_prediction_data_process(output_from_dataloader, self.device)
        return outputs



    def mysample(self, seed_edges):    # pylint: disable=arguments-differ
        pair_graph = self.g.edge_subgraph(
            seed_edges, relabel_nodes=False)
        eids = pair_graph.edata["_ID"]

        neg_graph = self.link_sampler._build_neg_graph(pair_graph, seed_edges)
        pair_graph, neg_graph = dgl.dataloading.compact_graphs([pair_graph, neg_graph])

        pair_graph.edata["_ID"] = eids
        seed_nodes = pair_graph.ndata["_ID"]

        sg = dgl.to_block(pair_graph)
        
        # exclude_eids = find_exclude_eids(
        #     g, seed_edges, exclude, reverse_eids, reverse_etypes,
        #     output_device)

        # input_nodes, _, blocks = sampler.sample(g, seed_nodes, exclude_eids)
        input_nodes = None
        return None, pair_graph,neg_graph,sg #dgl.dataloading.set_edge_lazy_features(pair_graph,[])
