    ...
    for run in range(args.runs):
        ...
        for epoch in range(start_epoch, start_epoch + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                ...
                results = test(model, 'valid', predictor, feat, edge_feat, graph,  split_edge, args.batch_size)
                ...
                for key, valid_res in results.items():
                    ...
                    to_print = (
                        ...
                        f"Epoch: {epoch}, "
                        f"Loss: {loss}, "
                        f"Valid ({args.val_percent*10}%) [{key}]: {valid_res} \n"
                    )
        ...
        for _idx, eval_epoch in enumerate(idx_to_test):
            ...
            if eval_epoch not in tested:
                ...
                tested[eval_epoch] = (
                    test(model, 'final_val', predictor, feat, edge_feat, graph,  split_edge,  args.batch_size),
                    test(model, 'test', predictor, feat, edge_feat, graph,  split_edge,  args.batch_size),
                )
            ...
            for iter_metric in loggers.keys():
                ...
                print_log(
                    ...
                    f"Valid ({args.val_percent*10}%) [{iter_metric}]: "
